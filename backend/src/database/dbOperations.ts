...
export function ListMetadataByDate(start: Date, end: Date): Promise<db.IMetadataEntry[]> {
    if (process.env[db.TableName.METADATA] === undefined) return Promise.reject({ message: NO_TABLE_ERROR });
    return new Promise((resolve, reject) => {
        const ddb = _getDynamo();
        const queryParams = {
        ...
        ddb.query(queryParams, (err: any, data: any) => {
            if (err) {
                ...
                if (result.length < 1) reject({ message: 'no entries found' });
                else {
...
export function GetEntryByID(templateId: string, table: db.TableName): Promise<db.IMetadataEntry | db.IHTMLEntry> {
    ...
    if (process.env[envTableName] === undefined) return Promise.reject({ message: NO_TABLE_ERROR });
    return new Promise((resolve, reject) => {
        const ddb = _getDynamo();
        const queryParams = {
